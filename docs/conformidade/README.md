...
| G5 | **Chaves no mesmo disco dos dados** | 21 `.env` em `~/docker` com `RAILS_MASTER_KEY`, `SECRET_KEY_BASE`, `LOCKBOX_MASTER_KEY`, senha do banco. **21 de 23 sem permissão 600** | curto prazo: `chmod 600`. Depois: **secret do Swarm** (`/run/secrets`, cifrado no Raft, fora de `docker inspect`) — as stacks de infra já usam, nenhum app Rails usa; e o diretório de segredos dentro do LUKS. Variável de ambiente expõe o segredo a quem tem acesso ao Docker |
